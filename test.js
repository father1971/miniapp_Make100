...
function generateExpressions(nums) {
  ...
  for (let i = 1; i < nums.length; i++) {
    ...
    for (let l of lefts) {
      for (let r of rights) {
        results.push({ val: l.val + r.val, expr: `${l.prec < 1 ? \`(\${l.expr})\` : l.expr}+\${r.prec < 1 ? \`(\${r.expr})\` : r.expr}`, prec: 1 });
        results.push({ val: l.val - r.val, expr: `${l.prec < 1 ? \`(\${l.expr})\` : l.expr}-\${r.prec <= 1 ? \`(\${r.expr})\` : r.expr}`, prec: 1 });
        results.push({ val: l.val * r.val, expr: `${l.prec < 2 ? \`(\${l.expr})\` : l.expr}*\${r.prec < 2 ? \`(\${r.expr})\` : r.expr}`, prec: 2 });
